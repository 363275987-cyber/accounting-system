<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-xl font-bold text-gray-800">📑 其他应收款管理</h1>
      <button
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
      ...
      <div class="bg-white rounded-xl border border-gray-100 p-4">
        <div class="text-sm text-gray-500 mb-1">⏳ 未收回</div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 mb-6">
      <div class="flex flex-wrap items-center gap-3">
        ...
        <select v-model="filterStatus" class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <option value="partial">部分收回</option>
          <option value="recovered">已收回</option>
        </select>
        <input
          v-model="searchText"
          type="text"
    ...
    <div v-else-if="filteredList.length === 0" class="text-center py-20">
      ...
      <div class="text-gray-500">暂无应收款记录</div>
      ...
      <table class="w-full">
        <thead>
          <tr class="bg-gray-50">
            ...
            <th class="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">预计收回日</th>
            <th class="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase">已收回</th>
            <th class="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase">未收回</th>
          ...
          <tr v-for="item in filteredList" :key="item.id" class="hover:bg-gray-50/50 transition">
            <td class="px-4 py-3">
              <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                :class="typeBadgeClass(item.receivable_type)">
                {{ typeLabel(item.receivable_type) }}
              </span>
            </td>
            ...
            <td class="px-4 py-3 text-sm text-green-600 text-right">{{ formatMoney(item.recovered_amount) }}</td>
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex items-center justify-end gap-2">
                ...
                <button @click="openRecoverDialog(item)" class="text-green-600 hover:text-green-800 text-xs cursor-pointer" v-if="item.status !== 'recovered'">收回</button>
    ...
    <div v-else class="space-y-4">
      <div v-for="item in filteredList" :key="item.id"
        ...
        <div class="flex items-center gap-4 text-xs mb-3">
          <span class="text-green-600">已收回 {{ formatMoney(item.recovered_amount) }}</span>
          <span class="text-orange-500">未收回 {{ formatMoney(item.remaining_amount) }}</span>
        </div>
        ...
        <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
          ...
          <button @click="openRecoverDialog(item)" class="text-green-600 text-xs cursor-pointer" v-if="item.status !== 'recovered'">收回</button>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="showModal = false">
      <div class="bg-white rounded-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <h2 class="text-lg font-bold text-gray-800 mb-4">{{ isEdit ? '编辑应收款' : '添加应收款' }}</h2>
        <div class="space-y-4">
          <div>
            <label class="block text-sm text-gray-600 mb-1">类型</label>
            <select v-model="form.receivable_type" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="deposit">押金</option>
              <option value="loan">借出款</option>
          ...
          <div>
            <label class="block text-sm text-gray-600 mb-1">预计收回日期（可选）</label>
    ...
    <!-- Recover Dialog -->
    <div v-if="showRecoverDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="showRecoverDialog = false">
      <div class="bg-white rounded-xl w-full max-w-md mx-4 p-6">
        <h2 class="text-lg font-bold text-gray-800 mb-4">收回 - {{ recoverTarget?.counterparty }}</h2>
        <div class="text-sm text-gray-500 mb-4">
          未收回金额: {{ formatMoney(recoverTarget?.remaining_amount) }}
        </div>
        <div>
          <label class="block text-sm text-gray-600 mb-1">收回金额</label>
          <input v-model.number="recoverAmount" type="number" min="0.01" :max="recoverTarget?.remaining_amount"
            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div class="flex justify-end gap-3 mt-6">
          <button @click="showRecoverDialog = false" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 cursor-pointer">取消</button>
          <button @click="handleRecover" :disabled="saving" class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-700 transition cursor-pointer disabled:opacity-50">
            {{ saving ? '处理中...' : '确认收回' }}
          </button>
        </div>
      </div>
...
const stats = computed(() => {
  ...
  return {
    ...
    recoveredAmount: items.reduce((s, i) => s + Number(i.recovered_amount || 0), 0),
    remainingAmount: items.reduce((s, i) => s + Number(i.remaining_amount || 0), 0),
    depositTotal: items.filter(i => i.receivable_type === 'deposit').reduce((s, i) => s + Number(i.amount || 0), 0),
    loanTotal: items.filter(i => i.receivable_type === 'loan').reduce((s, i) => s + Number(i.amount || 0), 0)
  }
})
...
const filteredList = computed(() => {
  ...
  if (filterType.value) {
    result = result.filter(i => i.receivable_type === filterType.value)
  }
...
async function handleSave() {
  ...
  const recoveredAmt = existing ? Number(existing.recovered_amount || 0) : 0
  const remainingAmount = Number(form.value.amount) - recoveredAmt

  const payload = {
    receivable_type: form.value.receivable_type,
  ...
  if (isEdit.value) {
    await supabase.from('other_receivables').update(payload).eq('id', editId.value)
  } else {
    payload.recovered_amount = 0
    payload.remaining_amount = payload.amount
    payload.status = 'pending'
    await supabase.from('other_receivables').insert(payload)
  }
...
function openRecoverDialog(item) {
  recoverTarget.value = item
  recoverAmount.value = Number(item.remaining_amount)
  showRecoverDialog.value = true
}

async function handleRecover() {
  if (!recoverTarget.value) return
  if (recoverAmount.value <= 0) return alert('请输入收回金额')
  if (recoverAmount.value > Number(recoverTarget.value.remaining_amount)) return alert('收回金额不能超过未收回金额')

  saving.value = true
  const item = recoverTarget.value
  const newRecovered = Number(item.recovered_amount) + recoverAmount.value
  const newRemaining = Number(item.amount) - newRecovered
  const newStatus = newRemaining <= 0 ? 'recovered' : 'partial'

  await supabase.from('other_receivables').update({
...
async function handleDelete(item) {
  if (!confirm(`确定要删除 ${item.counterparty} 的应收款记录吗？`)) return
  await supabase.from('other_receivables').update({
...
function typeLabel(type) {
  const map = { deposit: '押金', loan: '借出款', other: '其他' }
...
function statusLabel(status) {
  const map = { pending: '待收回', partial: '部分收回', recovered: '已收回' }
